...
class ExpNoiseRate():
    ...
    def get_files(self, path, suffix):
        ...
        for file in os.listdir(path):
            if file.endswith(suffix) and "tracked" not in file:
                file_pd = pd.read_csv(os.path.join(path, file))
                files_pd.append(file_pd)
                file_names.append(os.path.splitext(os.path.split(os.path.join(path, file))[1])[0])
    ...
    def save_results(self, directory, folder_name, save_fig):
        ...
        out_file_name = os.path.join(directory, folder_name, "cells_exp_noise_rate.txt")
        ...
        out_file_name = os.path.join(directory, folder_name, "cells_exp_noise_rate_mean.txt")
        ...
        out_file_name = os.path.join(directory, folder_name, "background.txt")
        ...
        if save_fig:
            for name, fig in zip(self.figure_names, self.figures):
                fig.savefig(os.path.join(directory, folder_name, "exp_noise_rate_" + name + ".pdf"),
